 ...
 */
define(['N/runtime', 'N/file', 'N/query', 'N/search', 'N/record', 'N/ui/serverWidget', '../../Library/handlebars'],
/**
 * @param {file} file
 * @param {search} search
 ...
 */
function(runtime, file, query, search, record, serverWidget, handlebars) {
    ...
    function onRequest(context) {
    	...
    	var idTobe = [];
    	var idInterested = [];
    	var sTemplate = file.load(181108);
    	...
    	if (sAction == 'backend') {
			...
			var src = search.create({
				...
				columns : [ 'custrecord_myc_thisisme',
						'custrecord_myc_workingtobe',
						'custrecord_myc_iamiterested' ],
			...
			handlebars.registerHelper('if_even', function(conditional) {
			...
			var srch = search.load({
				id : 'customsearch_joblevel_matrix' //Job Level Search Matrix
			});
			var arrColumns = [ '' ];
			var arrData = [];
			...
			//Data Level
			var srch = search.load({
				id : 'customsearch_jobprofilelevel_matrix'
			});
			var res = getAllResults(srch);
			res.forEach(function(result) {
				...
				var sStrategy = '';
				if (result.getText({
					name : 'custrecord_sr_wf_planning_strategy1',
					join : 'custrecord_jpl_jobprofile'
				}) != '') {
					sStrategy = 'Workforce Planning Primary Strategy: <br/>'
							+ result.getText({
								name : 'custrecord_sr_wf_planning_strategy1',
								join : 'custrecord_jpl_jobprofile'
							})
