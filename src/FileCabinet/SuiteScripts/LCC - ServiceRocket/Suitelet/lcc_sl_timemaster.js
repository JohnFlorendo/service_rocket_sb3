...
function(task,serverWidget,url,libHelper) {
    ...
    function buildUISelectAction(context) {
        ...
        var urlLink = libHelper.getURL('customscript_lcc_sl_timemaster','customdeploy_lcc_sl_timemaster');
        urlLink += '&updateall=T';
    ...
    function submitSingleUpdate(context) {
        ...
        var stHTML = '';
        try{
            var timeId = context.request.parameters['custpage_time_record'];
            if(timeId){
            ...
            if(timeId){
                ...
                stHTML +=  '<div class="content"><div class="title">Confirmation</div><div class="descr">Done create/update Time Master Record with Time Record ['+timeId+']</div></div></div></div>';
            }
        }catch (e) {
    ...
    function submitUpdateAll(context) {
        ...
        try{
            ...
            mrTask.scriptId = 'customscript_sr_time_charge_auto_map';
